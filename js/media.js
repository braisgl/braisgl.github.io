...
(function () {
  ...
  // Prefer standard youtube.com embed with explicit origin to satisfy identity checks
    ...
    if (isFile) {
      ...
      urlEmbed = `https://www.youtube.com/embed/${video.id}?rel=0&modestbranding=1&iv_load_policy=3`;
    } else {
      const originParam = encodeURIComponent(window.location.origin);
      urlEmbed = `https://www.youtube.com/embed/${video.id}?rel=0&modestbranding=1&iv_load_policy=3&origin=${originParam}`;
    }
  ...
  async function fetchYouTubeVideos() {
    ...
    if (!apiKey || (!channelId && !uploadsPlaylistId)) {
      console.warn('Falta apiKey o channelId/uploadsPlaylistId en YT_CONFIG');
      return null;
    }
    ...
    for (let i = 0; i < 2; i++) {
      const plUrl = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=${playlistId}&key=${apiKey}${pageToken ? `&pageToken=${pageToken}` : ''}`;
  ...
  const active = videos[0];
      setActive(active);
      renderList(videos, active.id);
    } catch (e) {
      console.error(e);
      // Fallback
      const active = fallbackVideos[0];
      setActive(active);
      renderList(fallbackVideos, active.id);
    }
